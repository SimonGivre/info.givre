...
public final class NetUtils {
	 ...
	 * <code>endPoint</code> is in format of <code>[hostname:]port</code>.
	 * It can be either a port number, or a hostname and port number 
	 * separated by the character ':'.<p>
	 * 
	 * For example, '<code>www.company.com:80</code>', '<code>1234</code>'
	 * and '<code>localhost:3344</code>' are all valid end points.
	 * 
	 * @param endPoint  the string to parse.
	 * @return  the {@linkplain InetSocketAddress} parsed from string.
	 * @exception  NumberFormatException  if the port number is not a integer.
	 */
    ...
    /**
     * Quietly close the a {@linkplain Socket} object. It tries
     * to shutdown the input and output of the socket before close.
	...
	/**
	 * Quietly shutdown the input of a {@linkplain Socket}.
	 * Exception are ignored.
	 * 
	 * @param socket  the socket to shutdown the input
	 */
	public static void shutdownInput(Socket socket) {
		if (socket==null) return;
		try {if (!socket.isInputShutdown())socket.shutdownInput();} 
		catch (Throwable e) {log.debug(e.getMessage(), e);}			
	}
	
	/**
	 * Quietly shutdown the output of a {@linkplain Socket}.
	 * Exceptions are ignored.
	 * 
	 * @param socket  the socket to shutdown the output
	 * @see Socket#shutdownOutput()
	 */
	public static void shutdownOutput(Socket socket) {
		if (socket==null) return;
		try {if (!socket.isOutputShutdown()) socket.shutdownOutput();} 
		catch (Throwable e) {log.debug(e.getMessage(), e);}
	}
